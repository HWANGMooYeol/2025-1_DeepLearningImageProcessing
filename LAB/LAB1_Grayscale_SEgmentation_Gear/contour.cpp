...
int main()
{
    ...
    findContours(imthres, contours, hierarchy,RETR_EXTERNAL, CHAIN_APPROX_NONE, Point(0, 0));
   ...
   // circle(src, center, radius*0.805, Scalar(255, 0, 0), 1, 4, 0);
    ...
    for (size_t i = 0; i < toothContours.size(); i++) {
        ...
        if (m.m00 != 0) {
            ...
            putText(result, to_string(int(tooth_area)), Point((center.x) * ratio + (cx-text_size.width / 2) * (1 - ratio), (center.y) * ratio + (cy+text_size.height / 2) * (1 - ratio)), FONT_HERSHEY_SIMPLEX, 0.5, color, 1.5);
    ...
    printf("----------------------------------------------------------------------------------------------\n"); 
    printf(" Teeth number: %zu\n Avg, Teeth Area: %f\n Defective Teeth: %d\n Diameter: %f\n Quality Inspection: %s\n", toothContours.size(), avg_area, Defective, radius * 0.805 *2, Quality_Inspection.c_str());
    printf("----------------------------------------------------------------------------------------------\n");
